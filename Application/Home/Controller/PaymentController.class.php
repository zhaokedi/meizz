...
class PaymentController extends BaseController {
    ...
    public function getCode(){
            ...
            $config_value['subject'] = $this->tpshop_config['shop_info_store_title'].'订单';
            $config_value['show_url'] = 'http://'.I('server.HTTP_HOST');
    ...
    public function getPay(){
        ...
        $pay_radio = $_REQUEST['pay_radio'];
        $config_value = parse_url_param($pay_radio); // 类似于 pay_code=alipay&bank_code=CCB-DEBIT 参数
        $config_value['subject'] = '美之钻珠宝-充值订单';
        $order['order_amount'] = $order['account'];
        $order['add_time'] = $order['ctime'];
    ...
    public function getCodediy(){
            ...
            $config_value['subject'] = $this->tpshop_config['shop_info_store_title'].'订单';
            $config_value['show_url'] = 'http://'.I('server.HTTP_HOST');
            $code_str = $this->payment->get_codediy($order,$config_value);
            //微信JS支付
            if($this->pay_code == 'weixin' && $_SESSION['openid'] && strstr($_SERVER['HTTP_USER_AGENT'],'MicroMessenger')){
               $code_str = $this->payment->getJSAPIdiy($order,$config_value);
               exit($code_str);
            }
            $this->assign('code_str', $code_str);
            $this->assign('order_id', $order_id);
    ...
    public function getCodedesign(){
        ...
        // 修改订单的支付方式
        $payment_arr = M('Plugin')->where("`type` = 'payment'")->getField("code,name");
//        M('apointment')->where("id = $order_id")->save(array('pay_code'=>$this->pay_code,'pay_name'=>$payment_arr[$this->pay_code]));
//        show_bug($order_id);
//        exit($order_id);
        $order = M('apointment_design')->where("id = $order_id")->find();

        // tpshop 订单支付提交
        $pay_radio = $_REQUEST['pay_radio'];
        $config_value = parse_url_param($pay_radio); // 类似于 pay_code=alipay&bank_code=CCB-DEBIT 参数
//        $config_value['subject'] = $this->tpshop_config['shop_info_store_title'].'订单';
        $config_value['subject'] ='设计师服务费订单';
        $config_value['show_url'] = 'http://'.I('server.HTTP_HOST');
        $order['order_amount']=$order['money'];
        $order['add_time']=$order['addtime'];
        $code_str = $this->payment->get_code($order,$config_value);
        //微信JS支付
        if($this->pay_code == 'weixin' && $_SESSION['openid'] && strstr($_SERVER['HTTP_USER_AGENT'],'MicroMessenger')){
            $code_str = $this->payment->getJSAPIn($order,$config_value);
            exit($code_str);
        }
        $this->assign('code_str', $code_str);
        $this->assign('order_id', $order_id);
        $this->assign('pay_code', $this->pay_code);
        $this->display('payment2');  // 分跳转 和不 跳转
    }
