...
class CartController extends BaseController {
    ...
    function ajaxAddCart()
    {
        ...
        $prepay_type=I("prepay_type");
        session('prepay_type',$prepay_type);
    ...
    public function ajaxCartList()
    {
        ...
        $where = " session_id = '$this->session_id' and isdiy='no' "; // 默认按照 session_id 查询
        $this->user_id && $where = " user_id = ".$this->user_id." and isdiy='no' "; // 如果这个用户已经等了则按照用户id查询
...
//        show_bug( $result['cartList']);
     ...
     * ajax 请求获取购物车列表
     */
    public function ajaxDiyCartList()
    {
		$reat = tpCache('shop_info.diyp');
        $post_goods_num = I("goods_num"); // goods_num 购物车商品数量
        $post_cart_select = I("cart_select"); // 购物车选中状态
        $where = " session_id = '$this->session_id' and isdiy='yes' "; // 默认按照 session_id 查询
        $this->user_id && $where = " user_id = ".$this->user_id." and isdiy='no' "; // 如果这个用户已经等了则按照用户id查询
        ...
        $result = $this->cartLogic->diycartList($this->user, $this->session_id,1,1); // 选中的商品
        /*if(empty($result['total_price']))
            $result['total_price'] = Array( 'total_fee' =>0, 'cut_fee' =>0, 'num' => 0);*/

        $this->assign('cartList', $result['cartList']); // 购物车的商品
        $this->assign('total_price', $result['total_price']); // 总计
		$this->assign('price_reat',ceil($result['total_price']['total_fee']*$reat/100));//预付金额
        $this->display('ajax_diycart_list');
    }
    /**
     * 购物车第二步确定页面
     */
    public function cart2()
    {
        ...
        $sql = "select c1.usetype,c1.discount,c1.name,c1.money,c1.use_end_time,c1.condition,c1.content, c2.* from __PREFIX__coupon as c1 inner join __PREFIX__coupon_list as c2  on c2.cid = c1.id and c1.type in(0,1,2,3) and order_id = 0  where c2.uid = {$this->user_id}  and ".time()." < c1.use_end_time and c1.condition <= {$result['total_price']['total_fee']}";
        $couponList = $Model->query($sql);
        foreach($couponList as $k=>$v){
            $couponList[$k]['ban']=$v['use_end_time']>time()?0:1;
        }
//        show_bug($couponList);exit;
     ...
     * 购物车第二步确定页面
     */
    public function diycart2()
    {
		$reat = tpCache('shop_info.diyp');
        if($this->user_id == 0)
            $this->error('请先登陆',U('Home/User/login'));

        if($this->cartLogic->diycart_count($this->user_id,1) == 0 )
            $this->error ('你的购物车没有选中商品','Cart/diycart');

        $result = $this->cartLogic->diycartList($this->user, $this->session_id,1,1); // 获取购物车商品
        $shippingList = M('Plugin')->where("`type` = 'shipping' and status = 1")->select();// 物流公司
...
//        show_bug($couponList);exit;
    ...
    public function diycart3(){
        ...
        if($this->cartLogic->diycart_count($this->user_id,1) == 0 ) exit(json_encode(array('status'=>-2,'msg'=>'你的购物车没有选中商品','result'=>null))); // 返回结果状态
	...
	// 订单满额优惠活动
        ...
        $car_price = array(
			...
			'price_reat'     => $result['result']['price_reat'], // 应付定金
        ...
        if($_REQUEST['act'] == 'submit_order')
        {
            ...
            $result = $this->cartLogic->addOrder($this->user_id,$address_id,$shipping_code,$invoice_title,$coupon_id,$car_price,'',0,'yes'); // 添加订单
            exit(json_encode($result));
        }
    ...
    public function cart3(){
		...
		$address = M('UserAddress')->where("address_id = $address_id")->find();
//		$order_goods = M('cart')->where("user_id = {$this->user_id} and selected = 1 and isdiy='no'")->select();
		$order_goods = M('cart')->where("user_id = {$this->user_id} and selected = 1")->select();
//        show_bug($order_goods);exit;
        $result = calculate_price($this->user_id,$order_goods,$shipping_code,0,$address[province],$address[city],$address[district],$pay_points,$user_money,$coupon_id,$couponCode);
//        show_bug($result);
		if($result['status'] < 0)
			exit(json_encode($result));
	// 订单满额优惠活动
        ...
        if($_REQUEST['act'] == 'submit_order')
        {
            if(empty($coupon_id) && !empty($couponCode))
               $coupon_id = M('CouponList')->where("`code`='$couponCode'")->getField('id');
            $result = $this->cartLogic->addOrder($heka,$this->user_id,$address_id,$shipping_code,$invoice_title,$coupon_id,$car_price); // 添加订单
            exit(json_encode($result));
        }
            $return_arr = array('status'=>1,'msg'=>'计算成功','result'=>$car_price); // 返回结果状态
            exit(json_encode($return_arr));
    ...
    * 设计师服务费支付页面
    */
    public function designcart4(){
        $appo_id = I('appo_id');
        $appointment = M('apointment_design')->where("id = $appo_id")->find();
        // 如果已经支付过的订单直接到订单详情页面. 不再进入支付页面
        if($appointment['pay_status'] == 1){
            $order_detail_url = U("Home/Index/designer_detail",array('id'=>$appointment['design_id']));
            header("Location: $order_detail_url");
        }
