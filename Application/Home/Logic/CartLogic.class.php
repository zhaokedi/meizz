...
class CartLogic extends RelationModel
{
     ...
     * @param type $prepay_type  是否付定金
     * @param type $user_id 用户id
     */
    function addCart($goods_id,$goods_num,$goods_spec,$session_id,$user_id = 0,$isdiy = 'no',$diytype = 0)
    {
		...
		if($user_id)
		 $where .= "  or user_id= $user_id and isdiy='$isdiy' and diytype = $diytype";
        ...
        $where = " goods_id = $goods_id and spec_key = '$spec_key' and isdiy='$isdiy' and diytype = $diytype"; // 查询购物车是否已经存在这商品
        if($user_id > 0)
        ...
        $data = array(
                    ...
                    'prepay_type'     => session('prepay_type')?:0,   // 是否预付
                    'prepay_money'    => $goods['prepay_price'],   // 预付金额
					...
					'isdiy'    => $isdiy,   // 是否定制
					'diytype'    => $diytype,   // 定制商品类型
        );


       // 如果商品购物车已经存在
       if($catr_goods && $isdiy=='no')
       {
            $result = M('Cart')->where("id =".$catr_goods[id])->save(array("goods_num"=> ($catr_goods['goods_num'] + $goods_num),"prepay_type" => session('prepay_type')?:0,"prepay_money" => $goods['prepay_price'])); // 数量相加
       ...
       else
       {
		     if($isdiy=='yes')
			 {
				 if($diytype==2 || $diytype==3)
				 {
		           $result = M("Cart")->where(" diytype =2")->delete(); // 删除戒托
				   $result = M("Cart")->where(" diytype =3")->delete(); // 删除吊坠
				 }else
				 {
					 $result = M("Cart")->where(" diytype =".$diytype)->delete(); // 删除裸钻
				 }
			 }
    ...
    function cartList($user = array() , $session_id = '', $selected = 0,$mode =0)
    {
        ...
        foreach ($cartList as $k=>$val){
            ...
            $image=M('spec_image')->where(array('spec_image_id'=>$val['spec_key'],'goods_id'=>$val['goods_id']))->find();
//            show_bug($image);
            $spec_image= $image['src']?$image['src']:'';
            $cartList[$k]['spec_image']=$spec_image;

        	$cartList[$k]['goods_fee'] = $val['goods_num'] * $val['member_goods_price'];   //商品小计
            if($val['prepay_type']==1)
                $cartList[$k]['goods_fee']=$val['goods_num']*$val['prepay_money'];
        	$cartList[$k]['store_count']  = getGoodNum($val['goods_id'],$val['spec_key']); // 最多可购买的库存数量
            $cartList[$k]['is_gift']  = $goods['is_gift'];
            $cartList[$k]['gift_link']  = unserialize($goods['gift_link']);
     ...
     * $mode 0  返回数组形式  1 直接返回result
     */
    function diycartList($user = array() , $session_id = '', $selected = 0,$mode =0)
    {
...
function cart_freight2($shipping_code,$province,$city,$district,$weight)
{
   ...
   return $freight;
    ...
    {
        $couponlist = M('cashier_wxcoupon_receive')->where("id = $coupon_id")->find(); // 获取用户的优惠券
        if(empty($couponlist)) {
            if($mode == 1) return 0;
            return array('status'=>1,'msg'=>'','result'=>0);
        }

        $coupon = M('cashier_wxcoupon')->where("id = {$couponlist['cid']}")->find(); // 获取 优惠券类型表
        $coupon['kqexpand']=unserialize( $coupon['kqexpand']);
        if($coupon['card_type']=2){
            $coupon['money'] =  $coupon['kqexpand']['discount']?$coupon['kqexpand']['discount'] : 0;
        }else{
            $coupon['money'] =  $coupon['kqexpand']['reduce_cost']?$coupon['kqexpand']['reduce_cost'] : 0;
        }

        if($mode == 1) return $coupon['money'];
        return array('status'=>1,'msg'=>'','result'=>$coupon['money'],'type'=>$coupon['card_type']);
    }
     ...
     */
    public function addOrder($heka,$user_id,$address_id,$shipping_code,$invoice_title,$coupon_id = 0,$car_price,$access_token,$price_reat=0,$iddiy='no')
    {
        ...
        if($shipping_code=='daodian'){
            $shipping['name']='到店自提';
        }elseif($shipping_code=='jishi'){
            $shipping['name']='即时送货';
        }
        $data = array(
                ...
                'ptaddress'            =>$heka['ptaddress'],//'跑腿目的地',
                'hkcontent'            =>$heka['hkcontent'],//'贺卡内容',
				...
				'isdiy'            =>$iddiy, // 是否是定制
                'prepay_type'=>$prepay_type,//'是否有预付定金商品',
        );
		...
		$order1 = M('Order')->where("user_id= $user_id and address='".$address['address']."' and order_sn_go IS NOT NULL")->find();
		if(empty($order1))
		{
			$order_list = M('Order')->where("user_id= $user_id and address='".$address['address']."'")->find();
			foreach($order_list as $key => $val)
			{
				$update['order_sn_go'] = $order['order_sn']."_1";
				$row = M('order')->where(array('order_id'=>$val['order_id']))->save($update);
			}
		}else
		{
			$update['order_sn_go'] = $order1['order_sn_go'];
        ...
        // 2修改优惠券状态
        if($coupon_id > 0){
            $cardcode=M('cashier_wxcoupon_receive')->where("id = $coupon_id")->getField('cardcode');
            $url="https://api.weixin.qq.com/card/code/consume?access_token=$access_token";
//            logResult1(var_export($url,true));
            $json='{
                    "code": "'.$cardcode.'"
                }';
...
//            logResult1(var_export($result,true));
            if($result['errcode']==0 && $result['errmsg'] =='ok' ){
                ...
                $data3['consumetime'] = time();
                $data3['status'] = 1;
                M('cashier_wxcoupon_receive')->where("id = $coupon_id")->save($data3);
                $cid = M('cashier_wxcoupon_receive')->where("id = $coupon_id")->getField('cid');
                M('cashier_wxcoupon')->where("id = $cid")->setInc('consumenum'); // 优惠券的使用数量加一
            }
        }
        // 1插入order_goods 表
//        $cartList = M('Cart')->where("user_id = $user_id and selected = 1")->select();
        foreach($cartList as $key => $val)
        {
           ...
           $data2['spec_key_name']      = $val['spec_key_name']; // 商品规格名称
            $data2['prepay_type']           = $val['prepay_type']; // 是否预付
            $data2['prepay_money']           = $val['prepay_money']; // 预付金额
    ...
    public function cart_count($user_id,$mode = 0){
        $count = M('Cart')->where("user_id = $user_id and selected = 1 and isdiy = 'no'")->count();
     ...
     * $mode 0  返回数组形式  1 直接返回result
     */
    public function diycart_count($user_id,$mode = 0){
        $count = M('Cart')->where("user_id = $user_id and selected = 1 and isdiy = 'yes'")->count();
        if($mode == 1) return  $count;
