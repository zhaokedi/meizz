...
class GoodsLogic extends RelationModel
{
   ...
   public function get_filter_spec($goods_id_arr,$filter_param,$action,$mode = 0)
   {
        ...
        foreach($spec_key as $k => $v)
        {
           ...
           $list_spec[$spec_item[$v]['spec_id']]['item'][] = array('key'=>$spec_item[$v]['spec_id'],'id'=>$spec_item[$v]['id'],'val'=>$v,'item'=>$spec_item[$v]['item'],'imgurl'=>$spec_item[$v]['imgurl'],'href'=>urldecode(U("Goods/$action",$filter_param,'')));
        }
 ...
 /**
 * 筛选的价格期间
 * @param type $goods_id_str 筛选的分类id
...
/**
 *  * 根据品牌或者价格条件筛选出 商品id
 * @param type $brand_id 筛选品牌id
 * @param type $price 筛选价格
 */
...
function getGoodsIdByCt($ct)
{
        ...
        {
            $ct = explode('-',$ct);
            $where .= " and goods_ct >= {$ct[0]} and goods_ct <= {$ct[1]} ";
        }
        $arr = M('goods')->where($where)->getField('goods_id',true);
        return $arr ? $arr : array();
}
