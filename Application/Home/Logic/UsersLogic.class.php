...
//        }
     ...
     * @param $validated 是否验证
     * @param $invite_id 邀请码
     * @return array
     */
    public function reg($username,$password,$password2,$validated=0,$invite_id){
    	$is_validated = 0 ;
        if(check_email($username)){
            $is_validated = 1;
            $map['email_validated'] = $validated;
            $map['nickname'] = $map['email'] = $username; //邮箱注册
        }

        if(check_mobile($username)){
            $is_validated = 1;
            $map['mobile_validated'] = $validated;
        ...
        $recommend_order = tpCache('recommend.recommend_reg');//推荐注册分成是否开启
        ...
        if($invite_id>0)
            accountLog2($invite_id,$user_id, 0,$invite['point'], '邀请注册赠送积分');
...
//        $type = I('get.type',0);
    ...
    public function get_coupon($user_id,$type =0 ){
        ...
        $sql = "SELECT l.*,c.name,c.money,c.use_end_time,c.condition FROM `__PREFIX__coupon_list`".
            " l LEFT JOIN `__PREFIX__coupon`".
            " c ON l.cid =  c.id WHERE l.uid = {$user_id} {$where}  ORDER BY c.use_end_time DESC,l.use_time LIMIT {$Page->firstRow},{$Page->listRows}";
        $logs = $this->query($sql);
     ...
     * @return array
     */
    public function add_bank($user_id,$bank_id=0,$data){
        $post = $data;
        $bank_code=$post['bank_code'];
        if($bank_id == 0)
        {
            $c = M('bank')->where("bank_code = $bank_code")->find();
            if($c)
                return array('status'=>-1,'msg'=>'银行卡已绑定过其他帐号,无法再次绑定','result'=>'');
        }
        //检查手机格式
        if($post['bank_name'] == '')
            return array('status'=>-1,'msg'=>'开户行不能为空','result'=>'');
        if($post['account_name'] == '')
            return array('status'=>-1,'msg'=>'开户名不能为空','result'=>'');
        if(!check_bankcode($post['bank_code']))
            return array('status'=>-1,'msg'=>'银行卡号格式有误','result'=>'');
        //编辑模式
        if($bank_id > 0){
//            $address = M('bank')->where(array('address_id'=>$bank_id,'user_id'=> $user_id))->find();
//            if($post['is_default'] == 1 && $address['is_default'] != 1)
//                M('bank')->where(array('user_id'=>$user_id))->save(array('is_default'=>0));
            $row = M('bank')->where(array('id'=>$bank_id,'user_id'=> $user_id))->save($post);
            if(!$row)
                return array('status'=>-1,'msg'=>'操作完成','result'=>'');
            return array('status'=>1,'msg'=>'编辑成功','result'=>'');
        }
        ...
        $post['user_id'] = $user_id;
        // 如果目前只有一个银行卡则改为默认银行卡
//        $c = M('bank')->where("user_id = {$post['user_id']}")->count();
//        if($c == 0)  $post['is_default'] = 1;
        $id = M('bank')->add($post);
        //如果设为默认地址
//        $insert_id = M()->getLastInsID();
...
//        $map['address_id'] = array('neq',$insert_id);
//        if($post['is_default'] == 1)
//            M('bank')->where($map)->save(array('is_default'=>0));
        if(!$id)
            return array('status'=>-1,'msg'=>'添加失败','result'=>'');
        return array('status'=>1,'msg'=>'添加成功','result'=>$id);
    }
    /**
     * 修改密码
     * @param $user_id  用户id
