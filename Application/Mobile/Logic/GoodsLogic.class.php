...
class GoodsLogic extends RelationModel
{
    ...
    public function goods_cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0)
    {
                ...
                $sql = "SELECT * FROM  __PREFIX__goods_category WHERE level=1 AND is_show=1 ORDER BY parent_id , sort_order ASC";
...
//                $goods_category = convert_arr_key($goods_category, 'id');
                ...
                {
                    $goods_categoryson = D('goods_category')->where(array('parent_id'=>$value['id'],'is_show'=>1))->select();
                    $goods_category[$key]['son']=$goods_categoryson;
//                    if($value['level'] == 1)
