 ...
 * @return bool
 */
function is_login($name='admin_id'){
 ...
 * @param type $type      类型
 */
function goods_thum_images($goods_id,$width,$height,$type=2){
    ...
    $image->thumb($width, $height,$type)->save($path.$goods_thumb_name,NULL,100); //按照原图的比例生成一个最大为$width*$height的缩略图并保存
 ...
 */
function send_email($to=false,$subject='',$content=''){
...
function sendSMS($mobile, $code)
{
    ...
    if ($resp->result->success)
    {
        ...
        $data = M('sms_log')->where("code = '$code' and add_time > ".(time() - $sms_time_out))->find();
        // 没有就插入验证码,供验证用
        ...
        if(empty($config)){
            ...
            if($res){
                foreach($res as $k=>$val){
					if($val['name']=="adstart_time" || $val['name']=="adend_time")
					{
						$config[$val['name']] = strtotime($val['value']);
					}else
					{
 ...
 * @return  bool
 */
function accountLog2($user_id,$sid, $user_money = 0,$pay_points = 0, $desc = '',$distribut_money = 0){
    /* 插入帐户变动记录 */
    $account_log = array(
        'user_id'       => $user_id,
        'sid'       => $sid,
...
function update_pay_status($order_sn,$pay_status = 1)
{
    ...
    function confirm_order($id){
        ...
        $recommend_order = tpCache('recommend.recommend_order');//推荐订单分成是否开启
        order_give($order);// 调用送礼物方法, 给下单这个人赠送相应的礼物
       $user= M('users')->find($order['user_id']);
        $invite=tpCache('invite');
        $commission=$invite['commission']*$order['order_amount']/100;
        if($user['invite_id']>0)
            accountLog2($user['invite_id'],$user['user_id'],$commission,0,'下级消费返还佣金');
        //分销设置
        ...
        if($user['oauth']== 'weixin')
        {
            ...
            $wx_content = "您的订单 {$order['order_sn']} 已经签收成功，如有疑问，请联系客服。";
            $jssdk->push_msg($user['openid'],$wx_content);
        }
//        $content = "美之钻订单签收成功"."<br>订单金额:".$order['order_amount']."<br>订单编号:".$order['order_sn']."<br>联系方式:".$order['mobile']."<br>联系人:".$order['consignee']."<br>签收时间:".date("Y-m-d H:i:s");//发送邮件提醒
//        send_email1('美之钻订单签收成功提醒',$content);
		...
		if(!empty($prominfo)){
			if($prominfo['goods_num'] == $prominfo['buy_num']){
                ...
                if($order_id_arr){
                    ...
                    if($goods_num < $prominfo['goods_num']){
...
function calculate_price($user_id=0,$order_goods,$shipping_code='',$shipping_price=0,$province=0,$city=0,$district=0,$pay_points=0,$user_money=0,$coupon_id=0,$couponCode='')
{
    ...
    $goods_arr = M('goods')->where("goods_id in(".  implode(',',$goods_id_arr).")")->getField('goods_id,weight,market_price,is_free_shipping'); // 商品id 和重量对应的键值对
        ...
        foreach($order_goods as $key => $val)
        {
            ...
            if(!array_key_exists('member_goods_price',$val))
            {
                ...
                if($val['prepay_type'] ==1)
                    $order_goods[$key]['member_goods_price'] = $val['prepay_money'];
            ...
            if($val['prepay_type'] ==1)
                $order_goods[$key]['goods_fee'] = $val['goods_num'] * $val['prepay_money'];
        ...
        {
            $res= $cartLogic->getCouponMoney($user_id, $coupon_id,0); // 下拉框方式选择优惠券
            $coupon_price =$res['result'];
        }
    ...
    if(!$res){
       ...
       $order_amount = $goods_price + $shipping_price - $coupon_price; // 应付金额 = 商品价格 + 物流费 - 优惠券
            ...
            $order_amount = $goods_price*$coupon_price/10 + $shipping_price  ; // 应付金额 = 商品价格 + 物流费 - 优惠券
            $coupon_price=$goods_price-$order_amount;
        }
    }
       ...
       $total_amount = $goods_price + $shipping_price;
        ...
        $result = array(
			...
			'price_reat'      => ceil($total_amount*$reat/100), // 商品定金
