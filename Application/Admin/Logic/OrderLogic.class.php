...
class OrderLogic extends RelationModel
{
     ...
     * 获取订单商品详情
     */
    public function getGoOrderGoods($order_sn_go){
        $sql = "SELECT g.*,o.*,(o.goods_num * o.member_goods_price) AS goods_total FROM __PREFIX__order_goods o ".
            "LEFT JOIN __PREFIX__goods g ON o.goods_id = g.goods_id WHERE o.order_id in (select order_id from __PREFIX__`order` where order_sn_go='$order_sn_go')";
        $res = $this->query($sql);
        return $res;
    }
	 /*
     ...
     * 获取订单信息
     */
    public function getGoOrderInfo($order_sn_go)
    {
        //  订单总金额查询语句
        $order = M('order')->where("order_sn_go = '$order_sn_go'")->find();
        $order['address2'] = $this->getAddressName($order['province'],$order['city'],$order['district']);
        $order['address2'] = $order['address2'].$order['address'];
        $order['orderList'] = M('order')->where("order_sn_go = '$order_sn_go'")->select();
        return $order;
    }
    /*
     * 根据商品型号获取商品
    ...
    public function deliveryHandle($data){
		...
		$content = "美之钻订单发货成功"."<br>订单金额:".$order['order_amount']."<br>订单编号:".$order['order_sn']."<br>联系方式:".$order['mobile']."<br>联系人:".$order['consignee']."<br>配送单号:".$data['invoice_no']."<br>发货时间:".date("Y-m-d H:i:s");//发送邮件提醒
//		send_email1('美之钻订单发货提醒',$content);
		// 如果有微信公众号 则推送一条消息到微信
		...
		if($user['oauth']== 'weixin')
		{
			...
			$wx_content = "您的订单 {$order['order_sn']} 已经发货，注意查收，配送单号：{$data['invoice_no']}!";
     ...
     * @param array $data  查询数量
     */
    public function mergedeliveryHandle($data){
		$order = $this->getGoOrderInfo($data['order_sn_go']);
		$orderGoods = $this->getGoOrderGoods($data['order_sn_go']);
		$selectgoods = $data['goods'];
		$data['order_id'] = 0;
		$data['order_sn'] = $order['order_sn_go'];
		$data['delivery_sn'] = $this->get_delivery_sn();
		$data['zipcode'] = $order['zipcode'];
		$data['user_id'] = $order['user_id'];
		$data['admin_id'] = session('admin_id');
		...
		$data['province'] = $order['province'];
		$data['city'] = $order['district'];
		$data['district'] = $order['order_sn_go'];
		$data['address'] = $order['address'];
		$data['shipping_code'] = $order['shipping_code'];
		$data['shipping_name'] = $order['shipping_name'];
		$data['shipping_price'] = $order['shipping_price'];
		...
		//M('order')->where("order_sn_go=".$data['order_sn_go'])->save($updata);//改变订单状态
		foreach ($order['orderList'] as $k=>$v){
		$thisGoods = $this->getOrderGoods($v['order_id']);
		$is_delivery=0;
		...
		if($is_delivery == count($thisGoods)){
			$updata['shipping_status'] = 1;
		}else if($is_delivery==0){
			$updata['shipping_status'] = 0;
		}else{
			$updata['shipping_status'] = 2;
		}
		$updata['shipping_status'] = 1;
		M('order')->where("order_id=".$v['order_id'])->save($updata);//改变订单状态
		$content = "美之钻订单发货成功"."<br>订单金额:".$v['order_amount']."<br>订单编号:".$v['order_sn']."<br>联系方式:".$v['mobile']."<br>联系人:".$v['consignee']."<br>配送单号:".$data['invoice_no']."<br>发货时间:".date("Y-m-d H:i:s");//发送邮件提醒
		send_email1('美之钻订单发货提醒',$content);
		}
		$s = $this->orderActionLog($order['order_id'],'delivery',$data['note']."【合并单号：".$data['order_sn_go']."】");//操作日志志
		return $s && $r;
    }
