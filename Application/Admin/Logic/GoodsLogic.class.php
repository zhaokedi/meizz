...
class GoodsLogic extends RelationModel
{
     ...
     * @return array 返回数组
     */
    public function getSpecItem1($spec_id)
    { 
        $model = M('SpecItem');        
        $arr = $model->where("spec_id = $spec_id")->select(); 
        //$arr = get_id_val($arr, 'id','item');        
     ...
     */
    public function updateSpecImg($idx, $imgurl)
    {
        $model = M('SpecItem'); 
        $model->execute("UPDATE __PREFIX__spec_item  SET imgurl = '$imgurl' WHERE id = $idx");                
        $spec = $model->where("id = $idx")->find(); // 从新找出他自己
         if($spec['imgurl']==$imgurl)
		 {
            $str = "succ";
		 }
		 else
