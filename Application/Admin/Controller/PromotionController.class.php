...
class PromotionController extends BaseController {
	...
	public function gift_pack_info(){
		if(IS_POST){
			$data = I('post.');
			$data['goods_id']=implode('|',I('goods_id'));
			$data['goods_name']=implode('|',I('goods_name'));
			$data['start_time'] = strtotime($data['start_time']);
			$data['end_time'] = strtotime($data['end_time']);
			if(empty($data['id'])){
				$r = M('gift_pack')->add($data);
				M('goods')->where("goods_id=".$data['goods_id'])->save(array('prom_id'=>$r,'prom_type'=>1));
				adminLog("管理员添加抢购活动 ".$data['name']);
			}else{
				$r = M('gift_pack')->where("id=".$data['id'])->save($data);
//				M('goods')->where("prom_type=1 and prom_id=".$data['id'])->save(array('prom_id'=>0,'prom_type'=>0));
//				M('goods')->where("goods_id=".$data['goods_id'])->save(array('prom_id'=>$data['id'],'prom_type'=>1));
			}
			if($r){
				$this->success('编辑抢购活动成功',U('Promotion/gift_pack'));
				exit;
			}else{
				$this->error('编辑抢购活动失败',U('Promotion/gift_pack'));
			}
		}
		$id = I('id');
		$info['start_time'] = date('Y-m-d');
		$info['end_time'] = date('Y-m-d',time()+3600*24*60);
		if($id>0){
			$info = M('gift_pack')->where("id=$id")->find();
			$info['goods_name']=explode('|',$info['goods_name']);
			$info['goods_id']=explode('|',$info['goods_id']);
			$arr=array();
			foreach($info['goods_id'] as $k=>$v){
			$arr[$info['goods_id'][$k]]=$info['goods_name'][$k];
			}
			$info['start_time'] = date('Y-m-d',$info['start_time']);
			$info['end_time'] = date('Y-m-d',$info['end_time']);
			$this->assign('arr',$arr);
	...
	public function gift_pack_del(){
		$id = I('del_id');
		if($id){
			M('gift_pack')->where("id=$id")->delete();
			M('goods')->where("prom_type=1 and prom_id=$id")->save(array('prom_id'=>0,'prom_type'=>0));
			exit(json_encode(1));
		}else{
			exit(json_encode(0));
