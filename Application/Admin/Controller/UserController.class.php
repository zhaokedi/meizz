...
class UserController extends BaseController {
    ...
    public function ruleHandle(){
        ...
        if($id){
            $r = M('user_rule')->where(array('id'=>$id))->save($data);
        }else{
            $r = M('user_rule')->add($data);
        }


//        if($data['act'] == 'del'){
//            $r = D('user_rule')->where('level_id='.$data['level_id'])->delete();
//            if($r) exit(json_encode(1));
//        }

        if($r){
            $this->success("操作成功",U('Admin/User/rule'));
        }else{
            $this->error("操作失败",U('Admin/User/rule'));
        }
    }
    /**
     * 搜索用户名
    ...
    **/
    ...
    public function add(){
        ...
        $res = M('users')->where("mobile=".$mobile)->select();
        if (empty($res)) {
            $r = D('users')->add($data);
            if ($r) {
    ...
    public function feedback(){
        $msg_type1=array('商品咨询','库存及配送','支付问题','发票及保修','促销及赠品');
        $feedback =  M('feedback');
        $where = "1 and f.parent_id =0 ";
        $condition = "1 and parent_id =0 ";
        $msg_type=I('msg_type',-1);
        if( $msg_type>=0){
            $where .= "and f.msg_type=".I('msg_type');
            $condition.=" and msg_type=".I('msg_type');
            $this->assign('msg_type',I('msg_type'));
        }
        $keywords = I('keywords',false);
        if($keywords){
            $where .= " and f.user_name like '%$keywords%'";
            $condition .= " and user_name like '%$keywords%'";
            $this->assign('keywords',$keywords);
        }

        $count = $feedback->where($condition)->count();// 查询满足要求的总记录数


        $Page = new \Think\Page($count,10);// 实例化分页类 传入总记录数和每页显示的记录数
//        $res = $goods_collect->where($where)->order('goods_id desc')->limit($Page->firstRow.','.$Page->listRows)->select();
        $where.=" and f.user_id = u.user_id ";

        $res = M()->table('feedback f,users u')->field('u.mobile,f.*')->where($where)->order('f.msg_id desc')->limit($Page->firstRow.','.$Page->listRows)->select();
//        show_bug($res);exit;
        foreach($res as $k=>$v){
            $res[$k]['msg_typename']=$msg_type1[$v['msg_type']];
        }
    ...
    public function fdHandle(){
        $data = I('post.');


        if($data['act'] == 'add'){
            $r = D('feedback')->add($data);
        }
        if($data['act'] == 'edit'){
            $r = D('feedback')->where('msg_id='.$data['msg_id'])->save($data);
        }

        if($data['act'] == 'del'){
            $r = D('feedback')->where('msg_id='.$data['del_id'])->delete();
            if($r) exit(json_encode(1));
        }
    ...
    public function fddetail(){
        ...
        if(!$res){
            exit($this->error('不存在该留言'));
        }
        if(IS_POST){
            $add['parent_id'] = $id;
            $add['msg_content'] = I('post.content');
