...
class OrderController extends BaseController {
    ...
    */
     ...
     * ajax 合併发货订单列表
    */
    public function ajaxmergedelivery(){
    	$orderLogic = new OrderLogic();
    	$condition = array();
		$where = " where order_status>=1";
		if(I('consignee'))
		{
			$where .=" and consignee like '%".trim(I('consignee'))."%'";
		}
		if(I('order_sn'))
		{
			$where .=" and order_sn like '%".trim(I('order_sn'))."%'";
		}
		if(empty(I('shipping_status')))
		{
    	...
    	//$shipping_status = I('shipping_status');
    	//$condition['shipping_status'] = empty($shipping_status) ? array('neq',1) : $shipping_status;
    	//$count = M('order')->where($condition)->group('order_sn_go')->select();
		
		$sql1 = "select order_sn_go,consignee,mobile,shipping_name,sum(shipping_price) shipping_price,pay_time,sum(total_amount) total_amount,shipping_status,order_id from __PREFIX__`order` $where group by order_sn_go";
    	$count = D()->query($sql1);
    	$Page  = new AjaxPage(count($count),10);
    	//搜索条件下 分页赋值
    	foreach($condition as $key=>$val) {
    		$Page->parameter[$key]   =   urlencode($val);
    	}
    	$show = $Page->show();
    	//$orderList = M('order')->where($condition)->limit($Page->firstRow.','.$Page->listRows)->order('add_time DESC')->group("order_sn_go")->select();
		$sql = "select order_sn_go,consignee,mobile,shipping_name,sum(shipping_price) shipping_price,pay_time,sum(total_amount) total_amount,shipping_status,order_id from __PREFIX__`order` $where group by order_sn_go order by add_time desc limit $Page->firstRow,$Page->listRows";
...
//        if($order['is_gift']==1){
//            $gift=M('gift_pack')->find($order['gift_id']);
...
//        }
     ...
     * @param int $id 订单id
     */
    public function mergeorder_print($order_sn_go,$printstr){
        $orderLogic = new OrderLogic();
        $order = $orderLogic->getGoOrderInfo($order_sn_go);
        $order['province'] = getRegionName($order['province']);
        $order['city'] = getRegionName($order['city']);
        $order['district'] = getRegionName($order['district']);
        $order['full_address'] = $order['province'].' '.$order['city'].' '.$order['district'].' '. $order['address'];
        $orderGoods = $orderLogic->getGoOrderGoods($order_sn_go);
        $shop = tpCache('shop_info');
        $this->assign('order',$order);
        $this->assign('shop',$shop);
		$this->assign('printstr',$printstr);
        $this->assign('orderGoods',$orderGoods);
        $this->display('mergeprint');
    }
	
	
	public function search_type(){
		$order_sn_go = I('order_sn_go');
		$this->assign('order_sn_go',$order_sn_go);
    	$this->display();
    }
     ...
     * 生成发货单
     */
    public function mergedeliveryHandle(){
        $orderLogic = new OrderLogic();
		$data = I('post.');
		$res = $orderLogic->mergedeliveryHandle($data);
		if($res){
			$this->success('操作成功',U('Admin/Order/mergedelivery_info',array('order_sn_go'=>$data['order_sn_go'])));
		}else{
			$this->success('操作失败',U('Admin/Order/mergedelivery_info',array('order_sn_go'=>$data['order_sn_go'])));
		}
    }
    ...
    public function delivery_info(){
	 ...
	 public function mergedelivery_info(){
    	$order_sn_go = I('order_sn_go');
    	$orderLogic = new OrderLogic();
    	$order = $orderLogic->getGoOrderInfo($order_sn_go);
    	$orderGoods = $orderLogic->getGoOrderGoods($order_sn_go);
    	$this->assign('order',$order);
    	$this->assign('orderGoods',$orderGoods);
		$delivery_record = M('delivery_doc')->where("order_id in (select order_id from `order` where order_sn_go='$order_sn_go') or order_sn='$order_sn_go'")->select();
		$this->assign('delivery_record',$delivery_record);//发货记录
    	$this->display();
    }
     ...
     * 发货单列表
     */
    public function mergedelivery_list(){
        $this->display();
    }
    ...
    public function ajax_return_list(){
        ...
        $where = " 1 = 1 ";
        $consignee && $where.= " and order.consignee like '%$consignee%' ";
        $order_sn && $where.= " and return_goods.order_sn like '%$order_sn%' ";
        empty($order_sn) && $where.= " and return_goods.status = '$status' ";
//        $count = M('return_goods')->where($where)->count();
//        $count = M('return_goods')->field('r.*')->join('order on return_goods.order_id =order.order_id')->where($where)->count();
        $count = M('return_goods')->field('return_goods.*,order.consignee')->join('LEFT JOIN `order` on return_goods.order_id =order.order_id')->where($where)->count();

        $Page  = new AjaxPage($count,13);
        $show = $Page->show();
//        $list = M('return_goods')->where($where)->order("$order_by $sort_order")->limit("{$Page->firstRow},{$Page->listRows}")->select();
        $list = M('return_goods')->field('return_goods.*,order.consignee')->join('LEFT JOIN `order` on return_goods.order_id =`order`.order_id')->where($where)->order("$order_by $sort_order")->limit("{$Page->firstRow},{$Page->listRows}")->select();
        foreach($list as $k=>$v){
...
//        array('退换','换货');
        ...
        if(IS_POST)
        {
            ...
            $data['is_money'] = I('is_money');
            $data['money'] = I('money');
            $data['service'] = I('service');
            $note ="售后:{$type_msg[$data['type']]}, 状态:{$status_msg[$data['status']]},处理备注：{$data['remark']}";
   ...
   {                
    ...
    public function return_money_log(){
//        $timegap = I('timegap');
//        if($timegap){
//            $gap = explode('-', $timegap);
//            $begin = strtotime($gap[0]);
...
//        show_bug($order_sn);
        ...
        $show = $Page->show();
        $list = $log->where($condition)->order('addtime desc')->limit($Page->firstRow.','.$Page->listRows)->select();
        $this->assign('list',$list);
	...
	$sql = "select *,FROM_UNIXTIME(add_time,'%Y-%m-%d') as create_time from __PREFIX__`order` $where order by order_id";
    	$orderList = D()->query($sql);   
    ...
    public function fenqi_del(){
        $id = I('get.id');
        M('fenqi')->where("id = $id")->delete();
        $this->success('成功删除!');
    }
    public function ajax_fenqi_list(){
        // 搜索条件
        $return_type=C('RETURN_TYPE');
//        $order_sn =  trim(I('order_sn'));
        ...
        empty($username) && $where.= " and status = '$status' ";
        $count = M('fenqi')->where($where)->count();
        $Page  = new AjaxPage($count,13);
        $show = $Page->show();
//        $list = M('return_goods')->where($where)->order("$order_by $sort_order")->limit("{$Page->firstRow},{$Page->listRows}")->select();
        $list = M('fenqi')->where($where)->order("$order_by $sort_order")->limit("{$Page->firstRow},{$Page->listRows}")->select();
     ...
     * 退换货操作
     */
    public function fenqi_info()
    {
        $id = I('id');
        $fenqi = M('fenqi')->where("id= $id")->find();

        $user = M('users')->where("user_id = {$fenqi[user_id]}")->find();
        $goods = M('goods')->where("goods_id = {$fenqi[goods_id]}")->find();

        if(IS_POST)
        {

            $data['status'] = I('status');
            $data['mremark'] = I('remark');
            $data['service'] = I('service');


            $result = M('fenqi')->where("id= $id")->save($data);
            if($result)
            {
                $this->success('修改成功!');
                exit;
