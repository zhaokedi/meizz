...
class GoodsController extends BaseController {
    ...
    public function addEditGoods(){
            $diycat_id = tpCache('shop_info.cat_id');
            $GoodsLogic = new GoodsLogic();                         
            $Goods = D('Goods'); //
            $type = $_POST['goods_id'] > 0 ? 2 : 1; // 标识自动验证时的 场景 1 表示插入 2 表示更新  
            $arr=array();
            $goods_ids=I('goods_ids');
            $goods_names=I('goods_names');
//        logResult1(var_export(I(),true));
            foreach($goods_ids as $k=>$v){
                $arr[$goods_ids[$k]]=$goods_names[$k];
            }
            //ajax提交验证
            ...
            {                
                ...
                if(!$Goods->create(NULL,$type))// 根据表单提交的POST数据创建数据对象                 
                {
                    ...
                    $Goods->gift_link=serialize($arr);
                    //  form表单提交
                   // C('TOKEN_ON',true);                                                            
                    //$Goods->on_time = time(); // 上架时间
					$Goods->on_time = strtotime($_POST['on_time']); // 上架时间
					$Goods->down_time = strtotime($_POST['down_time']); // 下架时间
            ...
            $goodsInfo = D('Goods')->where('goods_id='.I('GET.id',0))->find();
             $goodsInfo['gift_link']=unserialize($goodsInfo['gift_link']);
            ...
            $cat_list = M('goods_category')->where("parent_id = 0")->select(); // 已经改成联动菜单 
            $supply = M('supply')->select();
            $brandList = $GoodsLogic->getSortBrands();
            $goodsType = M("GoodsType")->select();            
            $categoryList = $GoodsLogic->getSortCategory();
            $this->assign('categoryList',$categoryList);//所有分类
             $brandList = $GoodsLogic->getSortBrands();
             $this->assign('brandList',$brandList);//所有品牌
            $this->assign('level_cat',$level_cat);
            $this->assign('level_cat2',$level_cat2);
            $this->assign('cat_list',$cat_list);
            $this->assign('brandList',$brandList);
            $this->assign('goodsType',$goodsType);
			$this->assign('diycat_id',$diycat_id);
            $this->assign('goodsInfo',$goodsInfo);  // 商品详情  
            $this->assign('supply',$supply);  //供货商
    ...
    public function updateField(){
        ...
        if ($_POST['value']>0) {
            $model->$primary['state'] = 0;
        }else{
            $model->$primary['state'] = 1;   //缺货
        }
    ...
    {
        $goods_id = $_GET['id'];
        M('Goods')->where('goods_id ='.$goods_id)->save(array('del'=>0));
    ...
    public function delGoods()
    {
        ...
        M('Goods')->where('goods_id ='.$goods_id)->save(array('del'=>1));
        $return_arr = array('status' => 1,'msg' => '操作成功','data'  =>'',);   //$return_arr = array('status' => -1,'msg' => '删除失败','data'  =>'',);        
        $this->ajaxReturn(json_encode($return_arr));
    }
     ...
     * 永久删除商品
     */
    public function foreverDelGoods(){
        $goods_id = $_GET['id'];
        // 删除此商品
    ...
    public  function addEditSpec(){
		   ...
		   $items1 = $GoodsLogic->getSpecItem1($id);
           $spec[items] = implode(PHP_EOL, $items); 
           $this->assign('spec',$spec);
           
           $goodsTypeList = M("GoodsType")->select();  
		   $this->assign('specitems',$items1);               
           $this->assign('goodsTypeList',$goodsTypeList);           
           $this->display('_spec');           
    }  
     ...
     * 更新商品规格图片
     */    
    public function ajaxUpdateSpecImg(){     
         $GoodsLogic = new GoodsLogic();
         $str = $GoodsLogic->updateSpecImg($_POST['idx'] ,$_POST['imgurl']);
         exit($str);   
    }
    ...
    public function search($name=''){
        $where = array(array('is_on_sale'=>1,'on_time'=>array('elt',time())));
        if($name){
           $where = array_merge(array('goods_name'=>array('like','%'.$name.'%')),$where);
        }
        $data = M('Goods')
            ->where($where)
            ->field('goods_id,goods_name')->limit(20)->select();
        $return_arr = array('status' => 1,'msg' => '成功','data'  =>$data);
     ...
     * 商品列表Excel导出
     */
    public function exportGoodList(){
        $cat_id = I('get.cat_id','');
        ...
        if($where['key_word']){
            $where['_string'] = "goods_name like '%$key_word%' or goods_sn like '%$key_word%'";
        }
        $where['del'] = 0;
        $field = 'goods_id,goods_name,goods_sn,cat_id,shop_price,store_count,is_on_sale,is_recommend,is_new,is_hot';
        $list = M('Goods')->where($where)->field($field)->select();
        $catList = D('goods_category')->select();
        header('Content-Type: application/vnd.ms-excel');
        header('Content-Disposition: attachment; filename=goods.csv');
        header('Pragma: no-cache');
        header('Expires: 0');
        $title = array('ID', '商品名称', '货号', '分类', '价格', '库存', '上架', '精品', '新品', '热卖');
        $data = array();
        foreach($list as $key => $item){
            $data[$key] = array(
                ...
                $item['goods_name'],
                $item['goods_sn'],
                $catList[$item['cat_id']]['name'],
                $item['shop_price'],
                $item['store_count'],
                $item['is_on_sale']==1?'是':'否',
    ...
    public function modifyGoods(){
        ...
        if(stripos($ids,',')){
            $idArr = explode(',',$ids);
            foreach($idArr as $id){
                $result = $GoodsModel->where(array(array('goods_id'=>$id)))->save($data);
            }
        }else{
            $result = $GoodsModel->where(array('goods_id'=>$ids))->save($data);
        }
        if($result){
            ...
            $return_arr = array('status' => 0,'msg' => '修改失败','');
            $this->ajaxReturn($return_arr);
            exit();
        }
    }
    ...
    public function uploadGoods(){
			...
			for ($i = 1; $i < $len_result; $i++) { //循环获取各字段值  
				$Goods->goods_name = iconv('gb2312', 'utf-8', $result[$i][0]); //中文转码  
				$Goods->goods_remark = iconv('gb2312', 'utf-8', $result[$i][1]);  
				$Goods->goods_sn = iconv('gb2312', 'utf-8', $result[$i][2]); 
				$Goods->spu = iconv('gb2312', 'utf-8', $result[$i][3]);  
				$Goods->sku = iconv('gb2312', 'utf-8', $result[$i][4]);
				$Goods->cat_id = $result[$i][5];  
				$Goods->extend_cat_id = $result[$i][6];  
				$Goods->brand_id = $result[$i][7]; 
				$Goods->supply_id = $result[$i][8]; 
				$Goods->shop_price = $result[$i][9];  
				$Goods->market_price = $result[$i][10]; 
				$Goods->cost_price = $result[$i][11];  
				$Goods->on_time = strtotime($on_time); // 上架时间
				$Goods->down_time = strtotime($down_time); // 下架时间
				$Goods->original_img = iconv('gb2312', 'utf-8', $result[$i][14]);  
				$Goods->weight = $result[$i][15];  
				$Goods->is_free_shipping = $result[$i][16];  
				$Goods->store_count = $result[$i][17];  
				$Goods->give_integral = $result[$i][18];  
				$Goods->is_on_sale = $result[$i][19];  
				$Goods->is_recommend = $result[$i][20];  
				$Goods->is_new = $result[$i][21];  
				$Goods->keywords = iconv('gb2312', 'utf-8', $result[$i][22]);
				$Goods->linked = iconv('gb2312', 'utf-8', $result[$i][23]);
				$Goods->goods_content = iconv('gb2312', 'utf-8', $result[$i][24]);
				$goods_id = $insert_id = $Goods->add(); // 写入数据到数据库
                $Goods->afterSave($goods_id);
			}  
			fclose($handle); //关闭指针  
			$this->success("导入成功", U('Goods/bulkUpload'));
    }
...
private function _inputCsv($handle) { 
    ...
    public function selectstate(){
        $goods_id = $_POST('goods_id');
        $result = M('Goods')->where("goods_id = $goods_id")->select();
