...
class UserController extends BaseController {
    ...
    public function reg(){
        ...
        if(check_mobile($username) && TpCache('sms.regis_sms_enable')){
            $code = I('post.code','');
            if(!$code)
                exit(json_encode(array('status'=>-1,'msg'=>'请输入短信验证码','result'=>'')));
        ...
        $data = $this->userLogic->reg($username,$password,$password2,$validated);
        exit(json_encode($data));
    }
    ...
    public function return_goods_status()
    {
        ...
        if(!empty($return_goods))
            exit(json_encode(array('status'=>1,'msg'=>'已经在申请退货中..','result'=>-1)));
         else
    ...
    public function return_goods()
    {
        ...
        $return_goods = M('return_goods')->where("order_id = $order_id and goods_id = $goods_id and spec_key = '$spec_key' status in(0,1)")->find();
        if(!empty($return_goods))
        {
